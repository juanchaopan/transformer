from torch import triu, ones, arange, exp, zeros, float32, log, tensor, sin, cos, device
...
class Encoder(Module):
    def __init__(self, d_model, d_ffn, num_heads):
        super().__init__()
        # attention
        self.attention = MultiheadAttention(d_model, num_heads, batch_first=True, device=dev)
        self.attention_norm = LayerNorm(d_model, device=dev)
        # ffn
        self.ffn = Sequential(
            Linear(d_model, d_ffn, device=dev),
            ReLU(),
            Linear(d_ffn, d_model, device=dev))
        self.ffn_norm = LayerNorm(d_model, device=dev)
...
class Decoder(Module):
    def __init__(self, d_model, d_ffn, num_heads):
        super().__init__()
        # self attention
        self.self_attention = MultiheadAttention(d_model, num_heads, batch_first=True, device=dev)
        self.self_attention_norm = LayerNorm(d_model, device=dev)
        # cross attention
        self.cross_attention = MultiheadAttention(d_model, num_heads, batch_first=True, device=dev)
        self.cross_attention_norm = LayerNorm(d_model, device=dev)
        # ffn
        self.ffn = Sequential(
            Linear(d_model, d_ffn, device=dev),
            ReLU(),
